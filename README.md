...
- **Frontend.** TypeScript with Vite and CodeMirror. The learner works through one kata at a time, with previous/next navigation and a hint that reveals the explanation. The panes follow the OS colour scheme: CodeMirror's default styling assumes a light background, so in dark mode the One Dark theme is applied instead, which would otherwise leave the caret and the token colours unreadable. A CodeMirror theme is a static extension, so it sits in a compartment that a media-query listener swaps when the OS theme changes, which keeps the learner's edits and cursor in place.
...
- Each kata is one Markdown file with frontmatter (title, subject, editable file, difficulty, concepts) and sections for the starting code, starting tests, reference solution, and explanation. A kata hands the learner exactly one buffer to edit, named by `editable` as either `source` or `spec`, and the `# solution` section holds the reference version of that one buffer.
...
- Buffers cross into the browser's Ruby VM base64-encoded, and decoding yields binary strings. The spec's string literals then carried a different encoding from the loaded source's, which made equal non-ASCII strings compare unequal and reported a passing suite as red. Both buffers are forced to UTF-8 after decoding. Only the browser path was affected, because the native harness passes the buffers as JSON.
